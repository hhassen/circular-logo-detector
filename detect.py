...
# The programs first extracts the circles (Hough Transform) on each frame, then compares each circle with the object using the SIFT detector.
...
def matches_number(sift,img,kp1,des1):
    ...
    img2 = crop_img # trainImage
    if np.shape(img2)[0]==0 or np.shape(img2)[1]==0 :
        return None
...
while True: 
    ...
    if np.count_nonzero(circles)!=0:
        ...
        for cercle in circles[0, :]:
            x,y,r = cercle.astype(int)
            ...
            if NB_matches is not None:
                ...
                if NB_matches>NB_matches_min:
                    draw_box(image,cercle)
                    csv_addrow(export_file,cercle,frameId)
